...
function sp_post($type=null,$v=null,$tag,$where=array(),$ispage,$pagesize=20,$pagesetting=array(),$pagetpl='{first}{prev}{liststart}{list}{listend}{next}{last}'){
    ...
    $order = !empty($tag['order']) ? $tag['order'] : 'date';

    switch($type){
        case 'keyword':
            $where['title'] = array('like','%' . $v . '%');//关键字
        case 'cid':
            $cid=intval($v);
            $catids=array();
            $categories=M("Categories")->field("category_id")->where("status=1 and ( category_id=$cid OR path like '%-$cid-%' )")->order('category_id asc')->select();
    ...
    if (!empty($tag['cid'])) {
        $where['category_id'] = array('in',$tag['cid']);
    }
    if (!empty($tag['ids'])) {
...
function sp_sql_post($tid,$tag){
	...
	$join = "".C('DB_PREFIX').'products as b on a.object_id =b.id';
	$join2= "".C('DB_PREFIX').'users as c on ( b.author = c.id OR b.user = c.id )';
	$category_relationships_model= M("CategoryRelationships");

	$post = $category_relationships_model->alias("a")->join($join)->join($join2)->field($field)->where($where)->find();
	return $post;
}
...
function sp_sql_pages($tag){
	...
	$pages=$products_model->field($field)->where($where)->order($order)->limit($limit)->select();
	return $pages;
}
 ...
 */
function sp_get_category($category_id){
	
	$categories=F('all_categories');
	if(empty($categories)){
		$category_obj= M("Categories");
		$categories=$category_obj->where("status=1")->select();
		$mcategories=array();
		...
		F('all_categories',$mcategories);
		return $mcategories["t$category_id"];
	}else{
		return $categories["t$category_id"];
	}
}
/**
 * 返回指定分类下的子分类
 ...
 */
function sp_get_child_categories($category_id){

	$category_id=intval($category_id);
	$category_obj = M("Categories");
	$categories=$category_obj->where("status=1 and parent=$category_id")->order("listorder asc")->select();
 ...
 * 
 */
function sp_get_categories($tag){
	
	$where=array();
	$tag=sp_param_lable($tag);
	$field = !empty($tag['field']) ? $tag['field'] : '*';
	$limit = !empty($tag['limit']) ? $tag['limit'] : '';
	$order = !empty($tag['order']) ? $tag['order'] : 'category_id';
	
	//根据参数生成查询条件
	$where['status'] = array('eq',1);
	
	if (isset($tag['ids'])) {
		$where['category_id'] = array('in',$tag['ids']);
	}
	...
	$category_obj= M("Categories");
	$categories=$category_obj->field($field)->where($where)->order($order)->limit($limit)->select();
	return $categories;
}
...
function sp_admin_get_tpl_file_list(){
	$template_path=C("SP_TMPL_PATH").C("SP_DEFAULT_THEME")."/Product/";
